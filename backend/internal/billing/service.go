...
func (s *Service) Sync(ctx context.Context) error {
	...
	buckets, err := s.collectBuckets(ctx, start, end, now)
	if err != nil {
		return markFailure(err)
	}
	events, detailsAvailable, err := s.collectEvents(ctx, start, end, aggregateQuotaPerUnit(buckets), now)
	if err != nil {
		return markFailure(err)
	}

	successState := baseState
	...
	successState.LastSuccessfulEndAt = &end
	successState.UpdatedAt = now
	var commitErr error
	if detailsAvailable {
		if repo, ok := s.repo.(detailRepository); ok {
			commitErr = repo.SettleAndReplaceWindowAndAdvanceWithEvents(billingSource, start, end, storage.BillingResolutionSeconds, buckets, events, successState, now)
		} else {
			commitErr = s.repo.SettleAndReplaceWindowAndAdvance(billingSource, start, end, storage.BillingResolutionSeconds, buckets, successState, now)
		}
	} else {
		commitErr = s.repo.SettleAndReplaceWindowAndAdvance(billingSource, start, end, storage.BillingResolutionSeconds, buckets, successState, now)
	}
...
func (s *Service) Rebuild(ctx context.Context, start, end time.Time) error {
	...
	for windowStart := start; windowStart.Before(end); {
		...
		buckets, err := s.collectBuckets(ctx, windowStart, windowEnd, now)
		if err != nil {
			return err
		}
...
func (s *Service) collectBuckets(ctx context.Context, start, end, now time.Time) ([]storage.NewAPIBillingBucket, error) {
	...
	if aggregate.QuotaPerUnit <= 0 {
		return nil, errors.New("new-api billing aggregate has invalid quota_per_unit")
	}
	...
	for _, item := range aggregate.Items {
		...
		if resolver, ok := s.repo.(mappingResolver); ok {
			...
			if resolveErr != nil {
				return nil, fmt.Errorf("resolve billing mapping: %w", resolveErr)
			}
			if mapping != nil {
		...
		if buildErr != nil {
			return nil, fmt.Errorf("build billing bucket: %w", buildErr)
		}
	...
	if resolver, ok := s.repo.(historicalCreditRateResolver); ok && len(buckets) > 0 {
		...
		if resolveErr != nil {
			return nil, fmt.Errorf("load historical billing credit rates: %w", resolveErr)
		}
		for i := range buckets {
...
func (s *Service) collectEvents(ctx context.Context, start, end time.Time, quotaPerUnit float64, now time.Time) ([]storage.NewAPIBillingEvent, bool, error) {
	...
	for page := 1; ; page++ {
		...
		if err != nil {
			if isUnsupportedDetailsEndpoint(err) {
				// Detail rows are an optional audit enhancement. Older NewAPI
				// deployments can still settle from the aggregate endpoint.
				return nil, false, nil
			}
...
func isUnsupportedDetailsEndpoint(err error) bool {
